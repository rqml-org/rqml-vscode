// Document View Webview Provider
// REQ-UI-002: Open RQML overview as rendered HTML document

import * as vscode from 'vscode';
import { getSpecService } from '../services/specService';
import { transformToDocumentView } from '../transformers/rqmlToDocument';
...
export class DocumentViewProvider {
  ...
  async show(): Promise<void> {
    ...
    // Send initial data
    await this.sendDocumentData();
  }

  /**
   * Send document data to the webview
   */
  private async sendDocumentData(): Promise<void> {
    ...
    try {
      const data = transformToDocumentView(state.document);
      this.panel.webview.postMessage({
        type: 'setDocument',
        payload: data
      });
      ...
      this.panel.webview.postMessage({
        type: 'error',
        payload: `Error transforming document: ${error}`
      });
    }
  }
