...
export class PptxGenerator implements ExportGenerator {
  async generate(
    ...
    // Footer slide
    this.renderFooterSlide(pptx, metadata, provenance);

    const output = Buffer.from((await pptx.write({ outputType: 'nodebuffer' })) as ArrayBuffer);
    ...
    return provenance ? normalizeOoxml(output, provenance.date) : output;
  }
  ...
  private renderFooterSlide(
    ...
    slide.addText(provenanceLine(provenance), {
